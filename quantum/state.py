...
def random_ket(n=1):
    """ Sample a Haar-random state vector ($2^{n+1}-1$ degrees of freedom). """
    return random_kets(n, 1)[0]

def random_kets(n, size=1, kind='fast'):
    ...
    If `kind='ortho'`, at most $2^n$ orthogonal states can be generated using QR decomposition.
    """
    ...
    if kind == 'fast':
        ...
        assert size <= 2**n, f"Can't generate more than 2**{n} orthogonal states"
...
def random_dm(n=1, rank='full'):
    ...
    kets = random_kets(n, rank, kind='ortho')
