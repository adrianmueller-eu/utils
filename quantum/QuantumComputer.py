...
from ..mathlib.matrix import normalize, is_unitary, is_hermitian, is_diag, trace_product, eigh, outer, is_isometry, kron_eye, random_unitary
...
class QuantumComputer:
    ...
    def random(self, qubits, kind='haar', depth=1, p=0.7):
        qubits, to_alloc = self._check_qubit_arguments(qubits, True)
        self._alloc_qubits(to_alloc)
        n = len(qubits)
        if kind in ['haar', 'gue']:
            # apply a random_unitary on qubits
            self(random_unitary(2**n, kind=kind), qubits)
        elif kind == 'clifford':
            # Clifford group is generated by H, S, and CNOT gates
            clifford_1q = ['h', 's', 'sdg', 'x', 'y', 'z']  # 1-qubit Clifford generators
            clifford_2q = ['cx', 'cz']                      # 2-qubit Clifford generators

            for _ in range(depth):
                available_qubits = qubits.copy()
                while available_qubits:
                    if len(available_qubits) == 1 or (len(available_qubits) > 1 and np.random.random() < p):
                        # Apply 1-qubit gate
                        gate = np.random.choice(clifford_1q)
                        target = np.random.choice(available_qubits)
                        getattr(self, gate)(target)
                        available_qubits.remove(target)
                    else:
                        # Apply 2-qubit gate
                        gate = np.random.choice(clifford_2q)
                        control, target = np.random.choice(available_qubits, size=2, replace=False)
                        getattr(self, gate)(control, target)
                        available_qubits.remove(control)
                        available_qubits.remove(target)
        elif kind == 'haar_elem':
            # Create a random unitary from composition of random 1-qubit unitaries and entangling gates
            for _ in range(depth):
                # Apply entangling gates between pairs of qubits
                perm = np.random.permutation(qubits)
                for i in range(0, len(perm)-1, 2):
                    if np.random.random() < p:
                        control, target = perm[i], perm[i+1]
                        self.cx(control, target)

                # Apply random single-qubit unitaries from Haar measure
                for q in qubits:
                    r = np.random.rand(3)
                    theta = 2*np.arcsin(np.sqrt(r[0]))
                    phi, lam = 2*np.pi*r[1:]
                    self.u(theta, phi, lam, q)
        else:
