...
def _test_channels():
    ...
    st = np.kron(subsystem_state, env_state)
    st_expect = partial_trace(U @ st, [0,1])                             # QC(st)(U)[0,1]
    st_actual = sum([K @ dm(subsystem_state) @ K.conj().T for K in Ks])  # QC(subsystem_state)(Ks)[0,1]
